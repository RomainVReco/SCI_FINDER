...
for ($i = 0; $i<3; $i++) {
    if (!empty($obj[$i]->formality->content->personneMorale->identite->formeJuridique))
        {
            if (strcmp($obj[$i]->formality->content->personneMorale->identite->formeJuridique,"Société civile immobilière") || 
            strcmp($obj[$i]->formality->content->personneMorale->identite->formeJuridique,"6540"))
            {
                $sci = new Sci();
